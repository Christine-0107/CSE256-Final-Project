...
def collate_fn_with_mask(batch):
    ...
    for entry in dict_data_bunch:
        ...
        elif entry == 'attention_mask' or entry == 'id'or entry =='input_ids':
            dict_data_bunch[entry] = pad_sequence(
                dict_data_bunch[entry], torch.as_tensor(0))
    # return {"img": images, **dict_data_bunch}
    
    return tuple([samples, targets, deepsolo_ocr, answers, item, {"img": images, **dict_data_bunch}])
...
def tensor_dict_from_dict_list(tensor_list: List[dict]):
    keys = [k for k in tensor_list[0].keys() if (k != 'phrase' and k != 'deepsolo_ocr' and k != 'answers')]
    tensor_dict = dict()
    for k in keys:
