...
#from models.pre_select import build_vgmodel
...
def main(args):
    ...
    #logging.basicConfig(filename='/home/yanruxue/latr-main/src/VLTVG/logs/binary_imdbs_06.log', level=logging.INFO, format=log_format)
    logging.basicConfig(filename='/home/yanruxue/lsf/logs/pre_select.log', level=logging.INFO, format=log_format)
    logging.info("sss")
    ...
    model_without_ddp.load_state_dict(torch.load('/home/yanruxue/latr-main/src/VLTVG/checkpoints/binary_512/epoch_11.pth'), strict=False)
    #model_without_ddp.load_state_dict(torch.load('/home/yanruxue/lsf/checkpoints/binary_imdbs/epoch_1.pth'), strict=False)
    ...
    processor = AutoProcessor.from_pretrained("google/vit-base-patch16-224-in21k") # 根据给定的Vit模型加载处理器，用于准备输入数据
    dataset_train = build_dataset(test=False, args=args, tokenizer=tokenizer, transform=processor, STVQA=True)
    dataset_val = build_dataset(test=True, args=args, tokenizer=tokenizer, transform=processor, STVQA=True)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        #loss, total_1, cnt_1, total_2, cnt_2 = epoch_eval(
            #model, criterion, postprocessor, data_loader_val, device, args.save_pred_path, epoch
        #)
        #logging.info("epoch:{}, loss:{}, total_1:{}, cnt_1:{}, total_2:{}, cnt_2:{}".format(epoch, loss, total_1, cnt_1, total_2, cnt_2))
        ...
        #save_path = "/home/yanruxue/latr-main/src/VLTVG/checkpoints/binary_imdbs/epoch_" + str(epoch) + '.pth'
        save_path = "/home/yanruxue/lsf/checkpoints/binary_imdbs/epochst_" + str(epoch) + '.pth'
