...
# 继承Pytorch的Dataset类，并实现__len__和__getitem__方法，自定义数据集
class TextVQA(Dataset):
    def __init__(self, base_img_path, json_df, ocr_json_df, tokenizer, transform=None, max_seq_length=-1, target_size=(1000, 1000),
                 pad_token_box=[0, 0, 0, 0], qa_box=[0, 0, 0, 0], STVQA = False, train_ds = False, val_ds = False, train_dataset = 'stvqa', val_dataset = 'textvqa'):
        '''
        Arguments:
        ...
        self.val_ds = val_ds
        # 如果是STVQA数据集，加载已有的npy文件；如果是TextVQA，使用传来的json
        # train_ds和val_ds只用来控制STVQA数据集的情况
        if self.STVQA:
            print("init STVQA dataset...")
            if train_ds:
                if self.train_dataset == 'stvqa':
                    self.imdb_file = np.load("/home/yanruxue/latr-main/src/new_latr/origin_deepsolo/deepsolo_origin_train.npy", allow_pickle=True)
                    #self.imdb_file = np.load("/home/yanruxue/latr-main/src/new_latr/rm_stvqa_train.npy", allow_pickle=True)
            ...
            if val_ds:
                if self.val_dataset == 'textvqa':
                    self.imdb_file = np.load("/home/yanruxue/latr-main/src/new_latr/origin_deepsolo/deepsolo_origin_val.npy", allow_pickle=True)
                    #self.imdb_file = np.load("/home/yanruxue/latr-main/src/new_latr/rm_stvqa_val.npy", allow_pickle=True)
                    self.base_img_path = "/home/yanruxue/latr-main/src/deepsolo/datasets/textocr/test_images"
                    #self.base_img_path = "/home/yanruxue/latr-main/src/deepsolo/datasets/totaltext1/test_images/"
    ...
    def order_ocr_tokens(self, boxes, words):
        order_list = [[] for i in range(200)]
        # print(words)
        # print(boxes)
        for i in range(len(boxes)):
            boxes[i].append(i)
            # print(boxes[i][1])
            # print(int(boxes[i][1] / 10))
    ...
    def __getitem__(self, index):
        if self.STVQA:
            ...
            return {'pixel_values': img, 'bbox': torch.tensor(boxes), 'input_ids': torch.tensor(tokenized_words), 'labels': torch.tensor(answer),
                    ...
                    '''
            return {'pixel_values': img, 'bbox': torch.tensor(boxes), 'input_ids': torch.tensor(tokenized_words), 'labels': torch.tensor(answer),
                    "attention_mask" : torch.tensor(attention_mask),'id': torch.as_tensor([index]), 'question': sample_entry['question'], 'ocr_tokens': words, 'answers':sample_entry['valid_answers'], 'imdb': sample_entry}
            ...
            return {'pixel_values': img, 'bbox': torch.tensor(boxes), 'input_ids': torch.tensor(tokenized_words), 'labels': torch.tensor(answer),
                    "attention_mask" : torch.tensor(attention_mask),'id': torch.as_tensor([index]), 'question': sample_entry['question'], 'ocr_tokens': words, 'answers':sample_entry['answers']}
                    '''
            return {'pixel_values': img, 'bbox': torch.tensor(boxes), 'input_ids': torch.tensor(tokenized_words), 'labels': torch.tensor(answer), 'question_id': sample_entry['question_id'],
                    "attention_mask" : torch.tensor(attention_mask),'id': torch.as_tensor([index]), 'question': sample_entry['question'], 'ocr_tokens': words, 'answers':sample_entry['answers']}
